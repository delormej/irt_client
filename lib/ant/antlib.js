...
const channelConfigs = new Array(null, null, null, null);
    ...
    if (channelConfigs[channelId] != null) {
        ...
        if (channelId != BG_SCANNING_CHANNEL_ID &&
                (channelConfigs[channelId].deviceId == 0 || 
                    channelConfigs[channelId].status == STATUS_TRACKING_CHANNEL)) {
            // Get the channel config if we're not tracking.
            console.log("requesting channel id");
...
function findFirstEmptyChannel(config) {
    let channelId = 1;
...
function findExistingChannel(config) {
    for(var i = 1; i < channelConfigs.length; i++) {
        if ((channelConfigs[i] != null) &&
                channelConfigs[i].deviceId == config.deviceId)
            return i;
    }
    return -1;
}
...
function getChannelId(config) {
    ...
    if (config.deviceId > 0) 
        channelId = findExistingChannel(config);
    if (channelId <= 0)
        channelId = findFirstEmptyChannel(config);
    return channelId;
}

function openChannel2(channelId, config) {
    const HP_CHANNEL_SEARCH_TIMEOUT = 10; // 10 == 25s
    const SEARCH_TIMEOUT_INFINITE = 0xFF; 
    const channelCallback = ffi.Callback('bool', [ 'uchar', 'uchar' ], 
        config.channelCallback);
    config.channelCallback = channelCallback;

    //channelConfigs[channelId].channelCallback = channelEventFunc;
    if (!antlib.ANT_AssignChannel(channelId, config.channelType, ANT_NETWORK))
        throw new Error('Unable to assign channel.');
    antlib.ANT_AssignChannelEventFunction(channelId, config.channelCallback, config.buffer) 
    ...
    if (!antlib.ANT_SetChannelSearchTimeout(channelId, HP_CHANNEL_SEARCH_TIMEOUT))
        throw new Error("Unable to disable search timeout.");
    if (!antlib.ANT_SetLowPriorityChannelSearchTimeout(channelId, SEARCH_TIMEOUT_INFINITE))
        throw new Error("Unable to set low priority channel search timeout.");   
...
function openChannel(config) {
    ...
    if (config.status == STATUS_UNASSIGNED_CHANNEL) {
        if (!antlib.ANT_SetLowPriorityChannelSearchTimeout(channelId, SEARCH_TIMEOUT_INFINITE))
            throw new Error("Unable to set low priority channel search timeout.");

        if (!antlib.ANT_SetChannelSearchTimeout(channelId, 2))
            throw new Error("Unable to disable search timeout.");
...
exports.openChannel2 = openChannel2;
