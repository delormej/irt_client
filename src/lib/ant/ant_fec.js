...
const AntFec = function() { 
    ...
    function updateChannelStatus(status) {
        ...
        self.emit('channel_status', status, FEC_CHANNEL_CONFIG.deviceId);        
    }

    function ensureDeviceTracking() {
        if (FEC_CHANNEL_CONFIG.status != antlib.STATUS_TRACKING_CHANNEL) 
            antlib.requestChannelId(FEC_CHANNEL_ID);
    }    
    ...
    function openChannel(deviceId) {
        ...
        fecChannelId = antlib.openChannel(FEC_CHANNEL_ID, FEC_CHANNEL_CONFIG);     
    }
